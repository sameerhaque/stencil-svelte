...
const MultiButtonComponent = class {
    ...
    componentWillLoad() {
        this.configurations = ItemUtils.ItemUtils.generateConfigurationFromItemOptions(this.element);
    }
    componentDidLoad() {
    ...
    render() {
        return (index.h(index.Host, { class: "sliderSelector" }, index.h("div", { class: "toggle-button full-width", role: "group" }, Array.from(this.configurations.values()).map((item, index$1) => index.h("div", { class: "button-wrapper", "data-key": index$1 }, index.h("button", { class: (item['selected'] ? 'active' : '') + ' btn', id: 'button-' + StringUtils.StringUtils.toCamelCase(item['label']), type: "button", disabled: item['disabled'], role: "button", "aria-pressed": "false", value: item['key'], "button-index": index$1, onClick: e => this.handleClick(e) }, item['label'])))), index.h("div", { class: "active-indicator", id: "active-indicator", style: this.styles })));
    }
...
exports.snt_multi_button = MultiButtonComponent;
