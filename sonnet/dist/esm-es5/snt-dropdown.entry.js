...
var DropdownComponent = /** @class */ (function () {
    function DropdownComponent(hostRef) {
        ...
        this.keyboardEventProcessor = KeyboardEventProcessorBuilder.newInstance()
            .withKeys([Keyboard.TAB, Keyboard.ESCAPE], function () { return _this.close(); })
            .withKeys([Keyboard.ENTER, Keyboard.SPACE], function () { return _this.toggle(); })
            .withKey(Keyboard.ARROW_UP, function () { return _this.processArrowKeys(NavigationDirectionEnum.BACKWARDS); })
            .withKey(Keyboard.ARROW_DOWN, function () { return _this.processArrowKeys(NavigationDirectionEnum.FORWARDS); })
            .build();
    }
    ...
    DropdownComponent.prototype.setValue = function (value) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                this.selectedItem = this.items.find(function (item) { return item.key == value; });
                return [2 /*return*/];
            });
        });
    };
    DropdownComponent.prototype.getValue = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this.selectedItem.key];
            });
        });
    };
    DropdownComponent.prototype.setIndex = function (index) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
    ...
    DropdownComponent.prototype.getIndex = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this.items.indexOf(this.selectedItem)];
            });
        });
    };
    DropdownComponent.prototype.handleModalOpenAndClose = function (opened) {
        if (opened) {
    ...
    DropdownComponent.prototype.handleSelectedItemChange = function () {
        this.hiddenEl.dispatchEvent(new CustomEvent('blur'));
        this.dropdownSelectionChange.emit();
    };
    DropdownComponent.prototype.componentWillLoad = function () {
        this.initItemConfiguration();
        this.selectedItem = this.items.find(function (item) { return item.selected == true; });
    };
    ...
    DropdownComponent.prototype.componentDidLoad = function () {
        ...
        window.addEventListener('click', function () {
            if (document.activeElement != _this.dropdownButtonElement) {
                _this.close();
            }
        });
    };
    DropdownComponent.prototype.handleClick = function (item) {
    ...
    DropdownComponent.prototype.render = function () {
        var _this = this;
        var selectedItemId = (this.selectedItem != undefined) ? this.name + "-item-" + this.selectedItem.value : "";
        var selectItemLabelId = this.name + "-dropdown-selected-item-label";
        return (h(Host, { class: "dropdown", id: this.name + "-dropdown" }, h("div", { class: "dropdown-container show-label-xs " + ((this.opened) ? 'opened' : ''), "register-callback": "DropdownComponent.registerContainer" // TODO: Should register itself with a container event
            ,
            onClick: this.toggle.bind(this), onKeyDown: this.processKeyboardEvent.bind(this) }, h("div", { id: this.name + "-button-dropdown", class: "btn-link dropdown-display-btn " + ((this.opened) ? 'open' : ''), role: "listbox", tabindex: "0", "aria-activedescendant": (this.selectedItem) ? selectedItemId : false, "aria-label": "" + this.labelText, "aria-multiselectable": "false", "aria-disabled": "false", "aria-required": "true", "aria-expanded": (this.opened) ? "true" : false, ref: function (thisElement) { return _this.dropdownButtonElement = thisElement; } }, h("div", { id: selectItemLabelId, ref: function (el) { return _this.textValueElement = el; }, class: "text-overflow single-line " + (this.selectedItem ? 'chosen' : '') }, (this.selectedItem) ? this.selectedItem.value : this.placeholderText), h("div", { class: "icon-container icon-font iconTriangle", "aria-hidden": "true", role: "presentation" })), h("div", { class: "xs-navbar-header options-header" }, h("label", { id: this.name + "-dropdown-label", class: "pull-left ng-binding" }, this.labelText), h("button", { type: "button", class: "modal-close btn btn-text btn-text-w-span xs-navbar--close-btn pull-right right-col-off no-link-animation", "aria-labelledby": this.name + "-close-label " + this.name + "-dropdown-label", tabindex: "0" }, h("span", { id: this.name + "-close-label", class: "strong label" }, "Close"), h("span", { class: "sr-only" }, "Close Icon"), h("snt-icon", { styleClasses: "iconNavClose" }))), h("ul", { id: this.name + "-insurance-type-dropdown-items", class: "items menu list-unstyled text-left", role: "listbox", tabindex: "0", "aria-labelledby": selectItemLabelId, "aria-activedescendant": selectedItemId }, this.items.map(function (item) { return h("li", { class: "dropdown--menu-item click text-left no-margin\n              " + ((_this.selectedItem) == item ? "selected" : "") + "\n              " + ((item == _this.currentItem) ? "highlighted" : "") + "\n              " + ((item.disabled == true) ? "disabled" : "") }, h("div", { id: selectedItemId, role: "option", tabindex: "-1", "aria-selected": (_this.selectedItem == item) ? "true" : false, onClick: function () { _this.handleClick(item); }, ref: function (thisElement) { return (_this.currentItem == item && thisElement.focus()); } }, item.value)); }))), h("input", { class: "hidden-textbox", tabindex: "-1", type: "text", "data-field": this.name, id: this.name, required: this.required, ref: function (el) { return _this.hiddenEl = el; }, value: (this.selectedItem) ? this.selectedItem.key : "" })));
    };
    DropdownComponent.prototype.initItemConfiguration = function () {
        var configurationObject = this.itemConfiguration || ItemUtils.generateConfigurationFromItemOptions(this.element);
        if (configurationObject) {
            var configurations = configurationObject.getConfigurations();
            if (configurations) {
                this.items = Array.from(configurations.values());
            }
        }
    };
    DropdownComponent.prototype.processArrowKeys = function (direction) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.isOpen()];
    ...
    DropdownComponent.prototype.getNextAvailableItem = function (direction, currentItem) {
        if (currentItem === void 0) { currentItem = this.currentItem; }
    ...
    DropdownComponent.prototype.processKeyboardEvent = function (evt) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
    ...
    Object.defineProperty(DropdownComponent.prototype, "element", {
