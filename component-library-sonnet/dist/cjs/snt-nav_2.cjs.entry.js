...
const NavItemComponent = class {
    constructor(hostRef) {
        ...
        this.navItemEvent = index.createEvent(this, "navItemEvent", 7);
        this.expanded = false;
        this.current = false;
    ...
    render() {
        ...
        const icon = [
            index.h("snt-icon", { styleClasses: this.iconClassName })
        ];
        const link = [
            index.h("snt-link", { href: this.href, target: this.target, class: navLinkClass, current: this.current, "aria-label": linkAriaLabel, "aria-current": true, "hidden-title": this.linkTitle, "is-external": this.isExternal }, this.iconClassName ? icon : "", this.linkTitle)
        ];
        const expander = (index.h("button", { type: "button", class: "expander", onClick: this.toggleExpanded.bind(this), "aria-expanded": this.expanded.toString(), "aria-haspopup": "true", "aria-label": `${this.linkTitle}`, "aria-owns": subnavId }, index.h("snt-icon", { styleClasses: (this.expanded) ? "icon-icon_nav_minus_inv" : "icon-icon_nav_plus" })));
        const subnav = [
            index.h("div", { class: "position-relative" }, [link, expander]),
            index.h("div", { "aria-label": "1 of 1", role: "menu", class: `dropdown-menu ${showClass}`, id: subnavId, "aria-labelledby": expanderId }, index.h("slot", null))
        ];
        const actionReference = (this.href) ? this.href.replace("action:", "") : "";
        const actionLink = index.h("snt-action-link", { "action-reference": actionReference, linkTitle: this.linkTitle, iconClassName: this.iconClassName });
        const custom = [
            index.h("slot", null)
        ];
        const template = {
        ...
        return (index.h(index.Host, { class: WebUtils.WebUtils.generateClasses([], this.styleClasses), role: "listitem" }, template[this.type]));
    }
...
const NavComponent = class {
    ...
    render() {
        return (index.h(index.Host, { class: WebUtils.WebUtils.generateClasses(["navbar-nav"], this.styleClasses), role: "list" }, index.h("slot", null)));
    }
    get element() { return index.getElement(this); }
};
...
exports.snt_nav = NavComponent;
exports.snt_nav_item = NavItemComponent;
