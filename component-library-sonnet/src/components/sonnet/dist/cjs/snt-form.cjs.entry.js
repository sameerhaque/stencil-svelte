...
const modalComponent = require('./modal-component-7d3831c0.js');
const _commonjsHelpers = require('./_commonjsHelpers-fc53004f.js');
require('./HtmlContentBuilder-acff4f73.js');
require('./index-aa081a2a.js');
const Search = require('./Search-a6b837de.js');
const recaptchaComponent = require('./recaptcha-component-ecb770d7.js');
...
class TemplateService {
    static generateContextForElement(element) {
        ...
        Array.from(element.attributes).forEach(function (attribute) {
            _context[StringUtils.StringUtils.toCamelCase(attribute.name)] = attribute.value;
        });
        return _context;
    }
...
class FormService {
        ...
        if (_dataErrorParent !== null) {
            ...
            _dataErrorMessages.forEach((_dataError) => {
                if (_activeDataError == null) {
                    ...
                    if (StringUtils.StringUtils.hasLength(regExValue)) {
                        if (StringUtils.StringUtils.isString(_value)) {
    ...
    static processDataAttribute(attributeName, context, value, dataError, dataErrorParent, dataPattern, isDataPatternMatch) {
        if (dataError.hasAttribute(attributeName)) {
            ...
            else if (StringUtils.StringUtils.notEmpty(dataPattern)) {
    ...
    static processDateExceedAttribute(attributeName, context, value, dataError, dataErrorParent, form) {
        if (dataError.hasAttribute(attributeName)) {
            ...
            if (StringUtils.StringUtils.isEmpty(dateString)) {
                return false;
            }
    ...
    static processDatePreceedAttribute(attributeName, context, value, dataError, dataErrorParent, form) {
        if (dataError.hasAttribute(attributeName)) {
            ...
            if (StringUtils.StringUtils.isEmpty(futureDateString)) {
                return false;
            }
...
const FormComponent = class {
    ...
    async openErrorModal(actionData, modalId) {
        Search.Action.openErrorModal(actionData, modalId);
    }
    async openSuccessModal(actionData, modalId) {
        Search.Action.openSuccessModal(actionData, modalId);
    }
    async reset() {
        this.formEl.reset();
        const recaptchaElement = this.formEl.querySelector(recaptchaComponent.RecaptchaComponent.TAG_NAME);
        if (recaptchaElement) {
    ...
    componentDidLoad() {
        ...
        _elements.forEach((_element) => {
            modalComponent.DomUtils.attachHandler(_element, "invalid", this.onInvalidateFormElementHandler.bind(this));
            modalComponent.DomUtils.attachHandler(_element, "change", this.onChangeInputBlankHandler.bind(this));
            if (_element.tagName !== "SELECT") {
                modalComponent.DomUtils.attachHandler(_element, "blur", this.onBlurFormElementHandler.bind(this));
            }
            if (_element.tagName === "SELECT" || _element.type === "checkbox") {
                modalComponent.DomUtils.attachHandler(_element, "change", this.onChangeValueFormElementHandler.bind(this));
            }
        });
        // Attach onSubmit to call callback handler
        if (this.submitCallBackhandler) {
            modalComponent.DomUtils.attachHandler(this.formEl, "submit", this.submitCallBackhandler.bind(this));
        }
    }
    async getData() {
        var _result = {
    ...
    onBlurFormElementHandler(event) {
        ...
        if (StringUtils.StringUtils.hasLength(_value)) {
            FormService.process(event);
        }
        else {
            var _form = _element.form;
    ...
    getPayloadNameForElement(element) {
        if (modalComponent.DomUtils.hasValidStringAttribute(element, FormAttributes.DATA_FIELD_ATTRUBITE_NAME)) {
            return element.getAttribute(FormAttributes.DATA_FIELD_ATTRUBITE_NAME);
        }
        else if (modalComponent.DomUtils.hasValidStringAttribute(element, "name")) {
            return element.getAttribute("name");
        }
        throw "Element " + element + " does not have a payload name.";
    }
    render() {
        return (index.h("form", { ref: (el) => (this.formEl = el), onSubmit: (e) => {
                this.submitForm(e);
            } }, index.h("slot", null)));
    }
};
...
exports.snt_form = FormComponent;
