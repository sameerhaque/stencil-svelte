...
const SearchComboboxComponent = class {
    ...
    async updateSearchSuggestions() {
        if (this.searchFunction) {
            this.SearchSuggestion = await this.searchFunction(this.input.value, LanguageUtils.LanguageUtils.isFrench() ? LanguageUtils.LanguageUtils.LANGUAGE_ID.fr : LanguageUtils.LanguageUtils.LANGUAGE_ID.en, 10);
            this.expanded = this.SearchSuggestion.length > 0;
        }
    }
